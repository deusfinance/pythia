...
def process_message(msg):
	...
	if data['type'] == 'new_request':
		...
		if request:
			...
			[price, error] = sources.get_symbol_price(request['symbol'], request['source'], request['timestamp'])
			if error:
				price('Error', error)
				return

			# calculate difference of request price and node estimated price
			price_diff = abs(price['price'] - request['price'])
			...
			if price_diff/request['price'] < float(os.getenv("PRICE_TOLERANCE")):
				...
				print("Reject signing of request %s with price %s. estimated price: %s" % (mongo_id, request['price'], price['price']))
